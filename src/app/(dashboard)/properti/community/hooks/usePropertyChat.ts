...
export function usePropertyChat(initialPropertyId?: string) {
  ...
  // Load room data or properties overview
  const loadRoomData = useCallback(async (targetPropertyId?: string, isSilentRefresh = false) => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setError(json.message || "Gagal memuat data komunitas kost.");
      ...
      // If user specifically requested or selected a property, set active room data
      if (targetPropertyId && data.property) {
  ...
  // Initial load: Only load property room if initialPropertyId was provided
  useEffect(() => {
  ...
  // Supabase Realtime Channel Subscription (Strictly scoped by propertyId)
  useEffect(() => {
    if (!propertyId) {
  ...
  // Select a property from ChatGroupListView to enter the roomchat
  ...
  // Return from roomchat back to ChatGroupListView
  const backToChatList = useCallback(() => {
    setPropertyId(undefined);
    setProperty(null);
    setMessages([]);
    setPinnedMessages([]);
    // Refresh properties list to get latest messages and unread counts
    loadRoomData(undefined, true);
  }, [loadRoomData]);
  ...
  const sendMessage = useCallback(
    ...
    [propertyId, currentUser, activeTenantsCount, managementCount, sending]
  );
  ...
  const deleteMessage = useCallback(
    async (messageId: string) => {
      if (!propertyId) return false;
      try {
        const res = await fetch(`/api/community/chat/${encodeURIComponent(messageId)}`, {
          method: "DELETE",
        });
        const json = await res.json();
        if (json.success) {
          setMessages((prev) =>
            prev.map((m) =>
              m.id === messageId
                ? { ...m, isDeleted: true, content: "🚫 Pesan ini telah dihapus" }
                : m
            )
          );
          setPinnedMessages((prev) => prev.filter((p) => p.id !== messageId));
  ...
  return {
    ...
    backToChatList,
    switchProperty,
    refresh: () => loadRoomData(propertyId, true),
  };
}
