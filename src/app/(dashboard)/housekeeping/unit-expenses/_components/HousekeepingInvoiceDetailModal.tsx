...
export default function HousekeepingInvoiceDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 overflow-y-auto animate-fadeIn">
      <div className="relative w-full max-w-2xl rounded-3xl bg-white shadow-2xl overflow-hidden border border-gray-100 my-8">
        {/* Header Modal */}
        <div className="bg-gradient-to-r from-[#242823] via-[#383E36] to-[#1C201C] p-6 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-[#8FA28A]/20 text-[#8FA28A] border border-[#8FA28A]/30">
        ...
        {/* Modal Content */}
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto text-gray-800">
          {/* Status & Quick Action Bar */}
          <div className="rounded-2xl border border-gray-100 bg-gray-50/80 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-wider text-gray-400">
                Status Pembayaran Saat Ini
              </p>
              <div className="mt-1">{renderStatusBadge(invoice.status)}</div>
            </div>
            ...
            {waLink && (invoice.status === "PENDING" || invoice.status === "OVERDUE") && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 text-xs font-bold shadow-md shadow-emerald-600/10 active:scale-95 transition-all"
              >
                <MessageSquare className="h-4 w-4" />
                <span>Pengingat WA Realtime</span>
              </a>
            )}
          </div>
            ...
            {invoice.paymentReceipt ? (
              <div className="space-y-2">
                ...
                >
                  ...
                  <span>Buka Foto Bukti Transfer dalam Ukuran Penuh</span>
                </a>
              </div>
            ) : (
              <div className="p-6 rounded-2xl border border-dashed border-gray-200 bg-gray-50 text-center text-xs text-gray-500 space-y-1">
                <p className="font-bold text-gray-700">Belum Ada Bukti Pembayaran Diunggah</p>
                <p className="text-[11px] text-gray-400">
                  {invoice.status === "PAID"
                    ? "Status dikonfirmasi lunas secara manual oleh pengelola."
                    : "Menunggu penghuni mengunggah struk transfer atau pembayaran tunai."}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-gray-50 px-6 py-4 border-t border-gray-100 flex justify-end">
          <button
            onClick={onClose}
            className="rounded-2xl border border-gray-200 bg-white px-5 py-2.5 text-xs font-bold text-gray-700 hover:bg-gray-100 transition-all cursor-pointer"
          >
            Tutup Rincian
          </button>
        </div>
